...
*/
(function(g,u){"function"===typeof define&&define.amd?define(["bytebuffer"],u):"function"===typeof require&&"object"===typeof module&&module&&module.exports?module.exports=u(require("bytebuffer"),!0):(g.dcodeIO=g.dcodeIO||{}).ProtoBuf=u(g.dcodeIO.ByteBuffer)})(this,function(g,u){var d={};d.ByteBuffer=g;d.Long=g.Long||null;d.VERSION="5.0.1";d.WIRE_TYPES={};d.WIRE_TYPES.VARINT=0;d.WIRE_TYPES.BITS64=1;d.WIRE_TYPES.LDELIM=2;d.WIRE_TYPES.STARTGROUP=3;d.WIRE_TYPES.ENDGROUP=4;d.WIRE_TYPES.BITS32=5;d.PACKABLE_WIRE_TYPES=
...
function(){return new ActiveXObject("Microsoft.XMLHTTP")}],d=null,g=0;g<b.length;g++){try{d=b[g]()}catch(n){continue}break}if(!d)throw Error("XMLHttpRequest is not supported");return d};b.fetch=function(d,p){p&&"function"!=typeof p&&(p=null);if(b.IS_NODE){var g=require("fs");if(p)g.readFile(d,function(b,d){b?p(null):p(""+d)});else try{return g.readFileSync(d)}catch(n){return null}}else{var h=b.XHR();h.open("GET",d,p?!0:!1);h.setRequestHeader("Accept","text/plain");"function"===typeof h.overrideMimeType&&
h.overrideMimeType("text/plain");if(p)h.onreadystatechange=function(){4==h.readyState&&(200==h.status||0==h.status&&"string"===typeof h.responseText?p(h.responseText):p(null))},4!=h.readyState&&h.send(null);else return h.send(null),200==h.status||0==h.status&&"string"===typeof h.responseText?h.responseText:null}};b.toCamelCase=function(b){return b.replace(/_([a-zA-Z])/g,function(b,d){return d.toUpperCase()})};return b}();d.Lang={DELIM:/[\s\{\}=;:\[\],'"\(\)<>]/g,RULE:/^(?:required|optional|repeated|map)$/,
...
BOOL:/^(?:true|false)$/i,ID:/^(?:[1-9][0-9]*|0|0[xX][0-9a-fA-F]+|0[0-7]+)$/,NEGID:/^\-?(?:[1-9][0-9]*|0|0[xX][0-9a-fA-F]+|0[0-7]+)$/,WHITESPACE:/\s/,STRING:/(?:"([^"\\]*(?:\\.[^"\\]*)*)")|(?:'([^'\\]*(?:\\.[^'\\]*)*)')/g,STRING_DQ:/(?:"([^"\\]*(?:\\.[^"\\]*)*)")/g,STRING_SQ:/(?:'([^'\\]*(?:\\.[^'\\]*)*)')/g};d.DotProto=function(b,d){function g(b,c){var a=-1,k=1;"-"==b.charAt(0)&&(k=-1,b=b.substring(1));if(d.NUMBER_DEC.test(b))a=parseInt(b);else if(d.NUMBER_HEX.test(b))a=parseInt(b.substring(2),16);
else if(d.NUMBER_OCT.test(b))a=parseInt(b.substring(1),8);else throw Error("illegal id value: "+(0>k?"-":"")+b);a=k*a|0;if(!c&&0>a)throw Error("illegal id value: "+(0>k?"-":"")+b);return a}function q(b){var c=1;"-"==b.charAt(0)&&(c=-1,b=b.substring(1));if(d.NUMBER_DEC.test(b))return c*parseInt(b,10);if(d.NUMBER_HEX.test(b))return c*parseInt(b.substring(2),16);if(d.NUMBER_OCT.test(b))return c*parseInt(b.substring(1),8);if("inf"===b)return Infinity*c;if("nan"===b)return NaN;if(d.NUMBER_FLT.test(b))return c*
...
c||"'"===c)this._stringOpen=c;return c};e.peek=function(){if(0===this.stack.length){var b=this.next();if(null===b)return null;this.stack.push(b)}return this.stack[0]};e.skip=function(b){var c=this.next();if(c!==b)throw Error("illegal '"+c+"', '"+b+"' expected");};e.omit=function(b){return this.peek()===b?(this.next(),!0):!1};e.toString=function(){return"Tokenizer ("+this.index+"/"+this.source.length+" at line "+this.line+")"};h.Tokenizer=a;var l=function(b){this.tn=new a(b);this.proto3=!1},e=l.prototype;
e.parse=function(){var b={name:"[ROOT]","package":null,messages:[],enums:[],imports:[],options:{},services:[]},c,a=!0,k;try{for(;c=this.tn.next();)switch(c){case "package":if(!a||null!==b["package"])throw Error("unexpected 'package'");c=this.tn.next();if(!d.TYPEREF.test(c))throw Error("illegal package name: "+c);this.tn.skip(";");b["package"]=c;break;case "import":if(!a)throw Error("unexpected 'import'");c=this.tn.peek();("public"===c||(k="weak"===c))&&this.tn.next();c=this._readString();this.tn.skip(";");
k||b.imports.push(c);break;case "syntax":if(!a)throw Error("unexpected 'syntax'");this.tn.skip("=");"proto3"===(b.syntax=this._readString())&&(this.proto3=!0);this.tn.skip(";");break;case "message":this._parseMessage(b,null);a=!1;break;case "enum":this._parseEnum(b);a=!1;break;case "option":this._parseOption(b);break;case "service":this._parseService(b);break;case "extend":this._parseExtend(b);break;default:throw Error("unexpected '"+c+"'");}}catch(e){throw e.message="Parse error at line "+this.tn.line+
...
throw Error("illegal value: "+c);};e._parseOption=function(b,c){var a=this.tn.next(),k=!1;"("===a&&(k=!0,a=this.tn.next());if(!d.TYPEREF.test(a))throw Error("illegal option name: "+a);var e=a;k&&(this.tn.skip(")"),e="("+e+")",a=this.tn.peek(),d.FQTYPEREF.test(a)&&(e+=a,this.tn.next()));this.tn.skip("=");this._parseOptionValue(b,e);c||this.tn.skip(";")};e._parseOptionValue=function(b,c){var a=this.tn.peek();if("{"!==a)n(b.options,c,this._readValue(!0));else for(this.tn.skip("{");"}"!==(a=this.tn.next());){if(!d.NAME.test(a))throw Error("illegal option name: "+
...
function(b){var c=this.tn.next();if(!d.NAME.test(c))throw Error("illegal rpc service method name: "+c);var a=c,k={request:null,response:null,request_stream:!1,response_stream:!1,options:{}};this.tn.skip("(");c=this.tn.next();"stream"===c.toLowerCase()&&(k.request_stream=!0,c=this.tn.next());if(!d.TYPEREF.test(c))throw Error("illegal rpc service request type: "+c);k.request=c;this.tn.skip(")");c=this.tn.next();if("returns"!==c.toLowerCase())throw Error("illegal rpc service request type delimiter: "+
c);this.tn.skip("(");c=this.tn.next();"stream"===c.toLowerCase()&&(k.response_stream=!0,c=this.tn.next());k.response=c;this.tn.skip(")");c=this.tn.peek();if("{"===c){for(this.tn.next();"}"!==(c=this.tn.next());)if("option"===c)this._parseOption(k);else throw Error("illegal rpc service token: "+c);this.tn.omit(";")}else this.tn.skip(";");"undefined"===typeof b.rpc&&(b.rpc={});b.rpc[a]=k};e._parseMessage=function(b,c){var a=!!c,k=this.tn.next(),e={name:"",fields:[],enums:[],messages:[],options:{},services:[],
oneofs:{}};if(!d.NAME.test(k))throw Error("illegal "+(a?"group":"message")+" name: "+k);e.name=k;a&&(this.tn.skip("="),c.id=g(this.tn.next()),e.isGroup=!0);k=this.tn.peek();"["===k&&c&&this._parseFieldOptions(c);for(this.tn.skip("{");"}"!==(k=this.tn.next());)if(d.RULE.test(k))this._parseMessageField(e,k);else if("oneof"===k)this._parseMessageOneOf(e);else if("enum"===k)this._parseEnum(e);else if("message"===k)this._parseMessage(e);else if("option"===k)this._parseOption(e);else if("service"===k)this._parseService(e);
else if("extensions"===k)e.hasOwnProperty("extensions")?e.extensions=e.extensions.concat(this._parseExtensionRanges()):e.extensions=this._parseExtensionRanges();else if("reserved"===k)this._parseIgnored();else if("extend"===k)this._parseExtend(e);else if(d.TYPEREF.test(k)){if(!this.proto3)throw Error("illegal field rule: "+k);this._parseMessageField(e,"optional",k)}else throw Error("illegal message token: "+k);this.tn.omit(";");b.messages.push(e);return e};e._parseIgnored=function(){for(;";"!==this.tn.peek();)this.tn.next();
...
h.toString=function(f){return(f?this.className+" ":"")+this.fqn()};h.build=function(){throw Error(this.toString(!0)+" cannot be built directly");};q.T=n;var a=function(f,b,a,c,d){n.call(this,f,b,a);this.className="Namespace";this.children=[];this.options=c||{};this.syntax=d||"proto2"},h=a.prototype=Object.create(n.prototype);h.getChildren=function(b){b=b||null;if(null==b)return this.children.slice();for(var a=[],c=0,d=this.children.length;c<d;++c)this.children[c]instanceof b&&a.push(this.children[c]);
return a};h.addChild=function(b){var a;if(a=this.getChild(b.name))if(a instanceof s.Field&&a.name!==a.originalName&&null===this.getChild(a.originalName))a.name=a.originalName;else if(b instanceof s.Field&&b.name!==b.originalName&&null===this.getChild(b.originalName))b.name=b.originalName;else throw Error("Duplicate name in namespace "+this.toString(!0)+": "+b.name);this.children.push(b)};h.getChild=function(b){for(var a="number"===typeof b?"id":"name",c=0,d=this.children.length;c<d;++c)if(this.children[c][a]===
b)return this.children[c];return null};h.resolve=function(b,a){var c="string"===typeof b?b.split("."):b,d=this,e=0;if(""===c[e]){for(;null!==d.parent;)d=d.parent;e++}do{do{if(!(d instanceof q.Namespace)){d=null;break}d=d.getChild(c[e]);if(!(d&&d instanceof q.T)||a&&!(d instanceof q.Namespace)){d=null;break}e++}while(e<c.length);if(null!=d)break;if(null!==this.parent)return this.parent.resolve(b,a)}while(null!=d);return d};h.qn=function(b){var a=[],c=b;do a.unshift(c.name),c=c.parent;while(null!==
c);for(c=1;c<=a.length;c++){var d=a.slice(a.length-c);if(b===this.resolve(d,b instanceof q.Namespace))return d.join(".")}return b.fqn()};h.build=function(){for(var b={},c=this.children,d=0,e=c.length,k;d<e;++d)k=c[d],k instanceof a&&(b[k.name]=k.build());Object.defineProperty&&Object.defineProperty(b,"$options",{value:this.buildOpt()});return b};h.buildOpt=function(){for(var b={},a=Object.keys(this.options),c=0,d=a.length;c<d;++c)b[a[c]]=this.options[a[c]];return b};h.getOption=function(b){return"undefined"===
...
typeof f||f&&f instanceof String||a(typeof f,"not a string"),""+f;case b.TYPES.bytes:return g.isByteBuffer(f)?f:g.wrap(f,"base64");case b.TYPES["enum"]:for(var l=this.resolvedType.getChildren(b.Reflect.Enum.Value),h=0;h<l.length;h++)if(l[h].name==f||l[h].id==f)return l[h].id;if("proto3"===this.syntax)return("number"!==typeof f||f===f&&0!==f%1)&&a(typeof f,"not an integer"),(4294967295<f||0>f)&&a(typeof f,"not in range for uint32"),f;a(f,"not a valid enum value");case b.TYPES.group:case b.TYPES.message:f&&
"object"===typeof f||a(typeof f,"object expected");if(f instanceof this.resolvedType.clazz)return f;if(f instanceof b.Builder.Message){var l={},h;for(h in f)f.hasOwnProperty(h)&&(l[h]=f[h]);f=l}return new this.resolvedType.clazz(f)}throw Error("[INTERNAL] Illegal value for "+this.toString(!0)+": "+f+" (undefined type "+this.type+")");};l.calculateLength=function(f,a){if(null===a)return 0;var c;switch(this.type){case b.TYPES.int32:return 0>a?g.calculateVarint64(a):g.calculateVarint32(a);case b.TYPES.uint32:return g.calculateVarint32(a);
...
l.valueToString=function(a){if(!this.isMapKey)throw Error("valueToString() called on non-map-key element");return this.type===b.TYPES.bytes?a.toString("binary"):a.toString()};q.Element=e;var s=function(b,c,d,e,k,l){a.call(this,b,c,d,e,l);this.className="Message";this.extensions=void 0;this.clazz=null;this.isGroup=!!k;this._fieldsByName=this._fieldsById=this._fields=null},l=s.prototype=Object.create(a.prototype);l.build=function(a){if(this.clazz&&!a)return this.clazz;a=function(b,a){function c(a,f,
d,e){if(null===a||"object"!==typeof a){if(e&&e instanceof b.Reflect.Enum){var m=b.Reflect.Enum.getName(e.object,a);if(null!==m)return m}return a}if(g.isByteBuffer(a))return f?a.toBase64():a.toBuffer();if(b.Long.isLong(a))return d?a.toString():b.Long.fromValue(a);var k;if(Array.isArray(a))return k=[],a.forEach(function(a,b){k[b]=c(a,f,d,e)}),k;k={};if(a instanceof b.Map){for(var m=a.entries(),l=m.next();!l.done;l=m.next())k[a.keyElem.valueToString(l.value[0])]=c(l.value[1],f,d,a.valueElem.resolvedType);
return k}var m=a.$type,l=void 0,h;for(h in a)a.hasOwnProperty(h)&&(m&&(l=m.getChild(h))?k[h]=c(a[h],f,d,l.resolvedType):k[h]=c(a[h],f,d));return k}var f=a.getChildren(b.Reflect.Message.Field),d=a.getChildren(b.Reflect.Message.OneOf),e=function(c,m){b.Builder.Message.call(this);for(var k=0,l=d.length;k<l;++k)this[d[k].name]=null;k=0;for(l=f.length;k<l;++k){var h=f[k];this[h.name]=h.repeated?[]:h.map?new b.Map(h):null;!h.required&&"proto3"!==a.syntax||null===h.defaultValue||(this[h.name]=h.defaultValue)}if(0<
arguments.length)if(1!==arguments.length||null===c||"object"!==typeof c||!("function"!==typeof c.encode||c instanceof e)||Array.isArray(c)||c instanceof b.Map||g.isByteBuffer(c)||c instanceof ArrayBuffer||b.Long&&c instanceof b.Long)for(k=0,l=arguments.length;k<l;++k)"undefined"!==typeof(h=arguments[k])&&this.$set(f[k].name,h);else this.$set(c)},m=e.prototype=Object.create(b.Builder.Message.prototype);m.add=function(c,f,d){var e=a._fieldsByName[c];if(!d){if(!e)throw Error(this+"#"+c+" is undefined");
...
if(!(e instanceof b.Reflect.Message.Field))throw Error(this+"#"+c+" is not a field: "+e.toString(!0));return this[e.name]};m.$get=m.get;for(var k=0;k<f.length;k++){var l=f[k];l instanceof b.Reflect.Message.ExtensionField||a.builder.options.populateAccessors&&function(b){var c=b.originalName.replace(/(_[a-zA-Z])/g,function(a){return a.toUpperCase().replace("_","")}),c=c.substring(0,1).toUpperCase()+c.substring(1),f=b.originalName.replace(/([A-Z])/g,function(a){return"_"+a}),e=function(a,c){this[b.name]=
...
e;c<d;c++)if(e=this.children[c],e instanceof t||e instanceof s||e instanceof w){if(a.hasOwnProperty(e.name))throw Error("Illegal reflect child of "+this.toString(!0)+": "+e.toString(!0)+" cannot override static property '"+e.name+"'");a[e.name]=e.build()}else if(e instanceof s.Field)e.build(),this._fields.push(e),this._fieldsById[e.id]=e,this._fieldsByName[e.name]=e;else if(e instanceof s.OneOf)this._oneofsByName[e.name]=e;else if(!(e instanceof s.OneOf||e instanceof k))throw Error("Illegal reflect child of "+
this.toString(!0)+": "+this.children[c].toString(!0));return this.clazz=a};l.encode=function(b,a,c){for(var e=null,d,k=0,l=this._fields.length,h;k<l;++k)d=this._fields[k],h=b[d.name],d.required&&null===h?null===e&&(e=d):d.encode(c?h:d.verifyValue(h),a,b);if(null!==e)throw b=Error("Missing at least one required field for "+this.toString(!0)+": "+e),b.encoded=a,b;return a};l.calculate=function(b){for(var a=0,c=0,e=this._fields.length,d,k;c<e;++c){d=this._fields[c];k=b[d.name];if(d.required&&null===
k)throw Error("Missing at least one required field for "+this.toString(!0)+": "+d);a+=d.calculate(k,b)}return a};l.decode=function(a,c,e){"number"!==typeof c&&(c=-1);for(var d=a.offset,k=new this.clazz,l,h,s;a.offset<d+c||-1===c&&0<a.remaining();){l=a.readVarint32();h=l&7;s=l>>>3;if(h===b.WIRE_TYPES.ENDGROUP){if(s!==e)throw Error("Illegal group end indicator for "+this.toString(!0)+": "+s+" ("+(e?e+" expected":"not a group")+")");break}if(l=this._fieldsById[s])l.repeated&&!l.options.packed?k[l.name].push(l.decode(h,
a)):l.map?(h=l.decode(h,a),k[l.name].set(h[0],h[1])):(k[l.name]=l.decode(h,a),l.oneof&&(h=k[l.oneof.name],null!==h&&h!==l.name&&(k[h]=null),k[l.oneof.name]=l.name));else switch(h){case b.WIRE_TYPES.VARINT:a.readVarint32();break;case b.WIRE_TYPES.BITS32:a.offset+=4;break;case b.WIRE_TYPES.BITS64:a.offset+=8;break;case b.WIRE_TYPES.LDELIM:l=a.readVarint32();a.offset+=l;break;case b.WIRE_TYPES.STARTGROUP:for(;p(s,a););break;default:throw Error("Illegal wire type for unknown field "+s+" in "+this.toString(!0)+
"#decode: "+h);}}a=0;for(c=this._fields.length;a<c;++a)if(l=this._fields[a],null===k[l.name])if("proto3"===this.syntax)k[l.name]=l.defaultValue;else{if(l.required)throw a=Error("Missing at least one required field for "+this.toString(!0)+": "+l.name),a.decoded=k,a;b.populateDefaults&&null!==l.defaultValue&&(k[l.name]=l.defaultValue)}return k};q.Message=s;var c=function(a,c,e,d,k,l,h,g,r,p){n.call(this,a,c,l);this.className="Message.Field";this.required="required"===e;this.repeated="repeated"===e;
this.map="map"===e;this.keyType=d||null;this.type=k;this.resolvedType=null;this.id=h;this.options=g||{};this.defaultValue=null;this.oneof=r||null;this.syntax=p||"proto2";this.originalName=this.name;this.keyElement=this.element=null;!this.builder.options.convertFieldsToCamelCase||this instanceof s.ExtensionField||(this.name=b.Util.toCamelCase(this.name))},l=c.prototype=Object.create(n.prototype);l.build=function(){this.element=new e(this.type,this.resolvedType,!1,this.syntax,this.name);this.map&&(this.keyElement=
...
a.remaining();case b.TYPES["enum"]:return 0!==a;case b.TYPES.message:return null!==a;default:return!0}};l.encode=function(a,c,e){if(null===this.type||"object"!==typeof this.type)throw Error("[INTERNAL] Unresolved type in "+this.toString(!0)+": "+this.type);if(null===a||this.repeated&&0==a.length)return c;try{if(this.repeated){var d;if(this.options.packed&&0<=b.PACKABLE_WIRE_TYPES.indexOf(this.type.wireType)){c.writeVarint32(this.id<<3|b.WIRE_TYPES.LDELIM);c.ensureCapacity(c.offset+=1);var k=c.offset;
for(d=0;d<a.length;d++)this.element.encodeValue(this.id,a[d],c);var l=c.offset-k,h=g.calculateVarint32(l);if(1<h){var s=c.slice(k,c.offset),k=k+(h-1);c.offset=k;c.append(s)}c.writeVarint32(l,k-h)}else for(d=0;d<a.length;d++)c.writeVarint32(this.id<<3|this.type.wireType),this.element.encodeValue(this.id,a[d],c)}else this.map?a.forEach(function(a,e,d){d=g.calculateVarint32(8|this.keyType.wireType)+this.keyElement.calculateLength(1,e)+g.calculateVarint32(16|this.type.wireType)+this.element.calculateLength(2,
...
typeof this.type)throw Error("[INTERNAL] Unresolved type in "+this.toString(!0)+": "+this.type);if(null===a||this.repeated&&0==a.length)return 0;var e=0;try{if(this.repeated){var d,k;if(this.options.packed&&0<=b.PACKABLE_WIRE_TYPES.indexOf(this.type.wireType)){e+=g.calculateVarint32(this.id<<3|b.WIRE_TYPES.LDELIM);for(d=k=0;d<a.length;d++)k+=this.element.calculateLength(this.id,a[d]);e+=g.calculateVarint32(k);e+=k}else for(d=0;d<a.length;d++)e+=g.calculateVarint32(this.id<<3|this.type.wireType),e+=
...
c,!0));return d}if(this.map){var k=e.defaultFieldValue(this.keyType);d=e.defaultFieldValue(this.type);a=c.readVarint32();if(c.remaining()<a)throw Error("Illegal number of bytes for "+this.toString(!0)+": "+a+" required but got only "+c.remaining());var l=c.clone();l.limit=l.offset+a;for(c.offset+=a;0<l.remaining();)if(c=l.readVarint32(),a=c&7,c>>>=3,1===c)k=this.keyElement.decode(l,a,c);else if(2===c)d=this.element.decode(l,a,c);else throw Error("Unexpected tag in map field key/value submessage");
return[k,d]}return this.element.decode(c,a,this.id)};q.Message.Field=c;l=function(a,b,e,d,k,l,h){c.call(this,a,b,e,null,d,k,l,h)};l.prototype=Object.create(c.prototype);q.Message.ExtensionField=l;q.Message.OneOf=function(a,b,c){n.call(this,a,b,c);this.fields=[]};var t=function(b,c,e,d,k){a.call(this,b,c,e,d,k);this.className="Enum";this.object=null};t.getName=function(a,b){for(var c=Object.keys(a),e=0,d;e<c.length;++e)if(a[d=c[e]]===b)return d;return null};(t.prototype=Object.create(a.prototype)).build=
function(a){if(this.object&&!a)return this.object;a=new b.Builder.Enum;for(var c=this.getChildren(t.Value),e=0,d=c.length;e<d;++e)a[c[e].name]=c[e].id;Object.defineProperty&&Object.defineProperty(a,"$options",{value:this.buildOpt(),enumerable:!1});return this.object=a};q.Enum=t;l=function(a,b,c,e){n.call(this,a,b,c);this.className="Enum.Value";this.id=e};l.prototype=Object.create(n.prototype);q.Enum.Value=l;var k=function(a,b,c,e){n.call(this,a,b,c);this.field=e};k.prototype=Object.create(n.prototype);
q.Extension=k;var w=function(b,c,e,d){a.call(this,b,c,e,d);this.className="Service";this.clazz=null};(w.prototype=Object.create(a.prototype)).build=function(a){return this.clazz&&!a?this.clazz:this.clazz=function(a,b){for(var c=function(b){a.Builder.Service.call(this);this.rpcImpl=b||function(a,b,c){setTimeout(c.bind(this,Error("Not implemented, see: https://github.com/dcodeIO/ProtoBuf.js/wiki/Services")),0)}},e=c.prototype=Object.create(a.Builder.Service.prototype),d=b.getChildren(a.Reflect.Service.RPCMethod),
k=0;k<d.length;k++)(function(a){e[a.name]=function(c,e){try{try{c=a.resolvedRequestType.clazz.decode(g.wrap(c))}catch(d){if(!(d instanceof TypeError))throw d;}if(null===c||"object"!==typeof c)throw Error("Illegal arguments");c instanceof a.resolvedRequestType.clazz||(c=new a.resolvedRequestType.clazz(c));this.rpcImpl(a.fqn(),c,function(c,d){if(c)e(c);else{null===d&&(d="");try{d=a.resolvedResponseType.clazz.decode(d)}catch(k){}d&&d instanceof a.resolvedResponseType.clazz?e(null,d):e(Error("Illegal response type received in service method "+
b.name+"#"+a.name))}})}catch(k){setTimeout(e.bind(this,k),0)}};c[a.name]=function(b,e,d){(new c(b))[a.name](e,d)};Object.defineProperty&&(Object.defineProperty(c[a.name],"$options",{value:a.buildOpt()}),Object.defineProperty(e[a.name],"$options",{value:c[a.name].$options}))})(d[k]);Object.defineProperty&&(Object.defineProperty(c,"$options",{value:b.buildOpt()}),Object.defineProperty(e,"$options",{value:c.$options}),Object.defineProperty(c,"$type",{value:b}),Object.defineProperty(e,"$type",{value:b}));
return c}(b,this)};q.Service=w;var v=function(a,b,c,e){n.call(this,a,b,c);this.className="Service.Method";this.options=e||{}};(v.prototype=Object.create(n.prototype)).buildOpt=h.buildOpt;q.Service.Method=v;h=function(a,b,c,e,d,k,l,h){v.call(this,a,b,c,h);this.className="Service.RPCMethod";this.requestName=e;this.responseName=d;this.requestStream=k;this.responseStream=l;this.resolvedResponseType=this.resolvedRequestType=null};h.prototype=Object.create(v.prototype);q.Service.RPCMethod=h;return q}(d);
d.Builder=function(b,d,g){function q(a){a.messages&&a.messages.forEach(function(b){b.syntax=a.syntax;q(b)});a.enums&&a.enums.forEach(function(b){b.syntax=a.syntax})}var n=function(a){this.ptr=this.ns=new g.Namespace(this,null,"");this.resolved=!1;this.result=null;this.files={};this.importRoot=null;this.options=a||{}},h=n.prototype;n.isMessage=function(a){return"string"!==typeof a.name||"undefined"!==typeof a.values||"undefined"!==typeof a.rpc?!1:!0};n.isMessageField=function(a){return"string"!==typeof a.rule||
"string"!==typeof a.name||"string"!==typeof a.type||"undefined"===typeof a.id?!1:!0};n.isEnum=function(a){return"string"===typeof a.name&&"undefined"!==typeof a.values&&Array.isArray(a.values)&&0!==a.values.length?!0:!1};n.isService=function(a){return"string"===typeof a.name&&"object"===typeof a.rpc&&a.rpc?!0:!1};n.isExtend=function(a){return"string"!==typeof a.ref?!1:!0};h.reset=function(){this.ptr=this.ns;return this};h.define=function(a){if("string"!==typeof a||!d.TYPEREF.test(a))throw Error("illegal namespace: "+
a);a.split(".").forEach(function(a){var b=this.ptr.getChild(a);null===b&&this.ptr.addChild(b=new g.Namespace(this,this.ptr,a));this.ptr=b},this);return this};h.create=function(a){if(!a)return this;if(Array.isArray(a)){if(0===a.length)return this;a=a.slice()}else a=[a];for(var e=[a];0<e.length;){a=e.pop();if(!Array.isArray(a))throw Error("not a valid namespace: "+JSON.stringify(a));for(;0<a.length;){var d=a.shift();if(n.isMessage(d)){var h=new g.Message(this,this.ptr,d.name,d.options,d.isGroup,d.syntax),
c={};d.oneofs&&Object.keys(d.oneofs).forEach(function(a){h.addChild(c[a]=new g.Message.OneOf(this,h,a))},this);d.fields&&d.fields.forEach(function(a){if(null!==h.getChild(a.id|0))throw Error("duplicate or invalid field id in "+h.name+": "+a.id);if(a.options&&"object"!==typeof a.options)throw Error("illegal field options in "+h.name+"#"+a.name);var b=null;if("string"===typeof a.oneof&&!(b=c[a.oneof]))throw Error("illegal oneof in "+h.name+"#"+a.name+": "+a.oneof);a=new g.Message.Field(this,h,a.rule,
a.keytype,a.type,a.name,a.id,a.options,b,d.syntax);b&&b.fields.push(a);h.addChild(a)},this);var r=[];d.enums&&d.enums.forEach(function(a){r.push(a)});d.messages&&d.messages.forEach(function(a){r.push(a)});d.services&&d.services.forEach(function(a){r.push(a)});d.extensions&&(h.extensions="number"===typeof d.extensions[0]?[d.extensions]:d.extensions);this.ptr.addChild(h);if(0<r.length){e.push(a);a=r;r=null;this.ptr=h;h=null;continue}r=null}else if(n.isEnum(d))h=new g.Enum(this,this.ptr,d.name,d.options,
d.syntax),d.values.forEach(function(a){h.addChild(new g.Enum.Value(this,h,a.name,a.id))},this),this.ptr.addChild(h);else if(n.isService(d))h=new g.Service(this,this.ptr,d.name,d.options),Object.keys(d.rpc).forEach(function(a){var b=d.rpc[a];h.addChild(new g.Service.RPCMethod(this,h,a,b.request,b.response,!!b.request_stream,!!b.response_stream,b.options))},this),this.ptr.addChild(h);else if(n.isExtend(d))if(h=this.ptr.resolve(d.ref,!0))d.fields.forEach(function(a){if(null!==h.getChild(a.id|0))throw Error("duplicate extended field id in "+
h.name+": "+a.id);if(h.extensions){var c=!1;h.extensions.forEach(function(b){a.id>=b[0]&&a.id<=b[1]&&(c=!0)});if(!c)throw Error("illegal extended field id in "+h.name+": "+a.id+" (not within valid ranges)");}var d=a.name;this.options.convertFieldsToCamelCase&&(d=b.Util.toCamelCase(d));var d=new g.Message.ExtensionField(this,h,a.rule,a.type,this.ptr.fqn()+"."+d,a.id,a.options),e=new g.Extension(this,this.ptr,a.name,d);d.extension=e;this.ptr.addChild(e);h.addChild(d)},this);else{if(!/\.?google\.protobuf\./.test(d.ref))throw Error("extended message "+
d.ref+" is not defined");}else throw Error("not a valid definition: "+JSON.stringify(d));h=d=null}a=null;this.ptr=this.ptr.parent}this.resolved=!1;this.result=null;return this};h["import"]=function(a,d){var h="/";if("string"===typeof d){b.Util.IS_NODE&&(d=require("path").resolve(d));if(!0===this.files[d])return this.reset();this.files[d]=!0}else if("object"===typeof d){var g=d.root;b.Util.IS_NODE&&(g=require("path").resolve(g));if(0<=g.indexOf("\\")||0<=d.file.indexOf("\\"))h="\\";g=g+h+d.file;if(!0===
this.files[g])return this.reset();this.files[g]=!0}if(a.imports&&0<a.imports.length){var c=!1;if("object"===typeof d){if(this.importRoot=d.root,c=!0,g=this.importRoot,d=d.file,0<=g.indexOf("\\")||0<=d.indexOf("\\"))h="\\"}else"string"===typeof d?this.importRoot?g=this.importRoot:0<=d.indexOf("/")?(g=d.replace(/\/[^\/]*$/,""),""===g&&(g="/")):0<=d.indexOf("\\")?(g=d.replace(/\\[^\\]*$/,""),h="\\"):g=".":g=null;for(var r=0;r<a.imports.length;r++)if("string"===typeof a.imports[r]){if(!g)throw Error("cannot determine import root");
var k=a.imports[r];if("google/protobuf/descriptor.proto"!==k&&(k=g+h+k,!0!==this.files[k])){/\.proto$/i.test(k)&&!b.DotProto&&(k=k.replace(/\.proto$/,".json"));var n=b.Util.fetch(k);if(null===n)throw Error("failed to import '"+k+"' in '"+d+"': file not found");if(/\.json$/i.test(k))this["import"](JSON.parse(n+""),k);else this["import"](b.DotProto.Parser.parse(n),k)}}else if(d)if(/\.(\w+)$/.test(d))this["import"](a.imports[r],d.replace(/^(.+)\.(\w+)$/,function(a,b,c){return b+"_import"+r+"."+c}));
else this["import"](a.imports[r],d+"_import"+r);else this["import"](a.imports[r]);c&&(this.importRoot=null)}a["package"]&&this.define(a["package"]);a.syntax&&q(a);var p=this.ptr;a.options&&Object.keys(a.options).forEach(function(b){p.options[b]=a.options[b]});a.messages&&(this.create(a.messages),this.ptr=p);a.enums&&(this.create(a.enums),this.ptr=p);a.services&&(this.create(a.services),this.ptr=p);a["extends"]&&this.create(a["extends"]);return this.reset()};h.resolveAll=function(){var a;if(null==
this.ptr||"object"===typeof this.ptr.type)return this;if(this.ptr instanceof g.Namespace)this.ptr.children.forEach(function(a){this.ptr=a;this.resolveAll()},this);else if(this.ptr instanceof g.Message.Field){if(d.TYPE.test(this.ptr.type))this.ptr.type=b.TYPES[this.ptr.type];else{if(!d.TYPEREF.test(this.ptr.type))throw Error("illegal type reference in "+this.ptr.toString(!0)+": "+this.ptr.type);a=(this.ptr instanceof g.Message.ExtensionField?this.ptr.extension.parent:this.ptr.parent).resolve(this.ptr.type,
!0);if(!a)throw Error("unresolvable type reference in "+this.ptr.toString(!0)+": "+this.ptr.type);this.ptr.resolvedType=a;if(a instanceof g.Enum){if(this.ptr.type=b.TYPES["enum"],"proto3"===this.ptr.syntax&&"proto3"!==a.syntax)throw Error("proto3 message cannot reference proto2 enum");}else if(a instanceof g.Message)this.ptr.type=a.isGroup?b.TYPES.group:b.TYPES.message;else throw Error("illegal type reference in "+this.ptr.toString(!0)+": "+this.ptr.type);}if(this.ptr.map){if(!d.TYPE.test(this.ptr.keyType))throw Error("illegal key type for map field in "+
this.ptr.toString(!0)+": "+this.ptr.keyType);this.ptr.keyType=b.TYPES[this.ptr.keyType]}"proto3"===this.ptr.syntax&&this.ptr.repeated&&void 0===this.ptr.options.packed&&-1!==b.PACKABLE_WIRE_TYPES.indexOf(this.ptr.type.wireType)&&(this.ptr.options.packed=!0)}else if(this.ptr instanceof b.Reflect.Service.Method)if(this.ptr instanceof b.Reflect.Service.RPCMethod){a=this.ptr.parent.resolve(this.ptr.requestName,!0);if(!(a&&a instanceof b.Reflect.Message))throw Error("Illegal type reference in "+this.ptr.toString(!0)+
": "+this.ptr.requestName);this.ptr.resolvedRequestType=a;a=this.ptr.parent.resolve(this.ptr.responseName,!0);if(!(a&&a instanceof b.Reflect.Message))throw Error("Illegal type reference in "+this.ptr.toString(!0)+": "+this.ptr.responseName);this.ptr.resolvedResponseType=a}else throw Error("illegal service type in "+this.ptr.toString(!0));else if(!(this.ptr instanceof b.Reflect.Message.OneOf||this.ptr instanceof b.Reflect.Extension||this.ptr instanceof b.Reflect.Enum.Value))throw Error("illegal object in namespace: "+
typeof this.ptr+": "+this.ptr);return this.reset()};h.build=function(a){this.reset();this.resolved||(this.resolveAll(),this.resolved=!0,this.result=null);null===this.result&&(this.result=this.ns.build());if(!a)return this.result;a="string"===typeof a?a.split("."):a;for(var b=this.result,d=0;d<a.length;d++)if(b[a[d]])b=b[a[d]];else{b=null;break}return b};h.lookup=function(a,b){return a?this.ns.resolve(a,b):this.ns};h.toString=function(){return"Builder"};n.Message=function(){};n.Enum=function(){};n.Service=
function(){};return n}(d,d.Lang,d.Reflect);d.Map=function(b,d){function g(b){var a=0;return{next:function(){return a<b.length?{done:!1,value:b[a++]}:{done:!0}}}}var q=function(b,a){if(!b.map)throw Error("field is not a map");this.field=b;this.keyElem=new d.Element(b.keyType,null,!0,b.syntax);this.valueElem=new d.Element(b.type,b.resolvedType,!1,b.syntax);this.map={};Object.defineProperty(this,"size",{get:function(){return Object.keys(this.map).length}});if(a)for(var e=Object.keys(a),l=0;l<e.length;l++){var g=
this.keyElem.valueFromString(e[l]),c=this.valueElem.verifyValue(a[e[l]]);this.map[this.keyElem.valueToString(g)]={key:g,value:c}}},n=q.prototype;n.clear=function(){this.map={}};n["delete"]=function(b){b=this.keyElem.valueToString(this.keyElem.verifyValue(b));var a=b in this.map;delete this.map[b];return a};n.entries=function(){for(var b=[],a=Object.keys(this.map),d=0,l;d<a.length;d++)b.push([(l=this.map[a[d]]).key,l.value]);return g(b)};n.keys=function(){for(var b=[],a=Object.keys(this.map),d=0;d<
a.length;d++)b.push(this.map[a[d]].key);return g(b)};n.values=function(){for(var b=[],a=Object.keys(this.map),d=0;d<a.length;d++)b.push(this.map[a[d]].value);return g(b)};n.forEach=function(b,a){for(var d=Object.keys(this.map),l=0,g;l<d.length;l++)b.call(a,(g=this.map[d[l]]).value,g.key,this)};n.set=function(b,a){var d=this.keyElem.verifyValue(b),l=this.valueElem.verifyValue(a);this.map[this.keyElem.valueToString(d)]={key:d,value:l};return this};n.get=function(b){b=this.keyElem.valueToString(this.keyElem.verifyValue(b));
return b in this.map?this.map[b].value:void 0};n.has=function(b){return this.keyElem.valueToString(this.keyElem.verifyValue(b))in this.map};return q}(d,d.Reflect);d.loadProto=function(b,g,p){if("string"===typeof g||g&&"string"===typeof g.file&&"string"===typeof g.root)p=g,g=void 0;return d.loadJson(d.DotProto.Parser.parse(b),g,p)};d.protoFromString=d.loadProto;d.loadProtoFile=function(b,g,p){g&&"object"===typeof g?(p=g,g=null):g&&"function"===typeof g||(g=null);if(g)return d.Util.fetch("string"===
typeof b?b:b.root+"/"+b.file,function(n){if(null===n)g(Error("Failed to fetch file"));else try{g(null,d.loadProto(n,p,b))}catch(h){g(h)}});var q=d.Util.fetch("object"===typeof b?b.root+"/"+b.file:b);return null===q?null:d.loadProto(q,p,b)};d.protoFromFile=d.loadProtoFile;d.newBuilder=function(b){b=b||{};"undefined"===typeof b.convertFieldsToCamelCase&&(b.convertFieldsToCamelCase=d.convertFieldsToCamelCase);"undefined"===typeof b.populateAccessors&&(b.populateAccessors=d.populateAccessors);return new d.Builder(b)};
d.loadJson=function(b,g,p){if("string"===typeof g||g&&"string"===typeof g.file&&"string"===typeof g.root)p=g,g=null;g&&"object"===typeof g||(g=d.newBuilder());"string"===typeof b&&(b=JSON.parse(b));g["import"](b,p);g.resolveAll();return g};d.loadJsonFile=function(b,g,p){g&&"object"===typeof g?(p=g,g=null):g&&"function"===typeof g||(g=null);if(g)return d.Util.fetch("string"===typeof b?b:b.root+"/"+b.file,function(n){if(null===n)g(Error("Failed to fetch file"));else try{g(null,d.loadJson(JSON.parse(n),
p,b))}catch(h){g(h)}});var q=d.Util.fetch("object"===typeof b?b.root+"/"+b.file:b);return null===q?null:d.loadJson(JSON.parse(q),p,b)};return d});
